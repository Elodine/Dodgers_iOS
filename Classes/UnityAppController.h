...
@interface UnityAppController : NSObject<UIApplicationDelegate>
{
    ...
    //
    // There's an additional view controller for managing autorotation. We switch to
    // it when the autorotating view controller is used and we detect that the current
    // orientation has been disabled. The controller is swapped with
    // _viewControllerForOrientation[0] immediately, so _secondaryAutorotatingViewController
    // is never the actual active controller and can be ignored for most purposes. Note that
    // [UIViewController attemptRotationToDeviceOrientation] is insufficient to force iOS to
    // change orientation in this circumstance.
#if !PLATFORM_TVOS
